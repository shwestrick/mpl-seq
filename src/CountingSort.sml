...
struct
  ...
  fun seqSort(In, Keys, numBuckets) =
    let
      val Counts = AS.full(Primitives.alloc (numBuckets+1))
      val Out = AS.full(Primitives.alloc (AS.length In))
    in
      seqSortInternal In Out Keys (Seq.subseq Counts (0,numBuckets)) true;
      AS.update(Counts, numBuckets, Seq.length In);
      (Out, Counts)
    end
  ...
  fun sort In Keys numBuckets =
    ...
    in
      if (numBlocks <= 1 orelse n < SeqThreshold) then
        ...
        val bucketOffsets =
            Seq.tabulate (fn i => if (i = numBuckets) then n
                                   else Seq.nth destOffsets (i * numBlocks))
                          (numBuckets+1)
      in
        (C, bucketOffsets)
      end
